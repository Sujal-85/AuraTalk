import { ArrowLeft, Video, Phone, Search, Trash2, MoreVertical } from "lucide-react";
...
const ChatHeader = ({
  ...
  return (
    <div className={`p-2.5 bg-base-100 relative select-none ${!showSearch ? ' border-b border-base-300' : ''}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button className="mr-1" onClick={() => setSelectedUser(null)}>
          ...
          <div>
            <h3 
              className="font-medium max-w-[120px] lg:max-w-[220px] truncate cursor-pointer hover:text-primary transition-colors hover:underline" 
        ...
        {selectedUser._id !== 'ai-bot' && (
          <div className="flex items-center gap-1 lg:gap-2 relative">
            ...
            <div className="hidden lg:block">
              <button
                className={`p-2 hover:bg-base-200 rounded-full ${showSearch ? "bg-base-200" : ""}`}
                title="Search"
            ...
            <div className="block lg:hidden relative">
              ...
              {showMenu && (
                <div className="absolute right-0 mt-2 w-48 bg-base-100 border border-base-300 rounded-lg shadow-lg z-50">
                  <button className="w-full text-left px-4 py-2 hover:bg-base-200" onClick={() => { setShowSearch(true); setShowMenu(false); }}>Search</button>
                  <button
                    className="w-full text-left px-4 py-2 hover:bg-base-200"
                    onClick={() => {
                      if (onTabChange) onTabChange('mediaDocsLinks');
                      setShowMenu(false);
                    }}
                  >
                    Media, Links and Docs
                  </button>
                  <button className="w-full text-left px-4 py-2 hover:bg-base-200 text-red-600" onClick={() => { handleDeleteChat(); setShowMenu(false); }}>Clear chat</button>
                  <button className="w-full text-left px-4 py-2 hover:bg-base-200" onClick={() => { setSelectedUser(null); setShowMenu(false); }}>Close chat</button>
                </div>
              )}
            </div>
          </div>
      ...
      {showSearch && (
        <hr className="w-full h-px bg-base-300 border-0 mt-[9px]" />
      )}
      {showSearch && (
        <div className="flex items-center gap-2 mt-2 justify-end">
          <input
            ref={inputRef}
            type="text"
            className="input rounded-xl w-48 sm:w-64 max-w-lg"
          ...
          <button className="btn btn-sm text-lg rounded-3xl" onClick={onPrevMatch}>&uarr;</button>
          <button className="btn btn-sm text-lg rounded-3xl" onClick={onNextMatch}>&darr;</button>
        </div>
      )}
      {/* User Profile Popup */}
      <UserProfilePopup 
